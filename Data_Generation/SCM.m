...
classdef SCM < handle
    ...
    methods
        ...
        function [] = init_d(obj)
            ...
            tdy = obj.tx_ant(3) * obj.lamda; % self.lamda : wave length   
        ...
        function [] = def_pow(obj)
            ...
            % obj.pdp 정의 여부 확인
            if isempty(obj.pdp) == 1 % Power Delay Profile (PDP)
            ...
            % obj.n_ray 정의 여부 확인
            if isempty(obj.n_ray) == 1 % 채널의 path당 ray의 수
        ...
        function [subpath_coeff] = ray_cal(obj, sample_len, ZoD, AoD, ZoA, AoA, xpr, vel)
            ...
            % 직교 방사패턴 간섭 계산
            % eq 7.5-22 P1
            trx_coef = [ obj.rx_theta(ZoA); obj.rx_phi(AoA) ].'; %F_tx_u_t / F_rx_u_t...
            
            
            % eq 7.5-22 P2
            if xpr == 0
                trx_coef = trx_coef * ( exp(2j*pi*rand(1)) .* [1 0; 0 -1] );
            else
                trx_coef = trx_coef * ( exp(2j*pi*rand(2)) .* [1 1/sqrt(xpr); 1/sqrt(xpr) 1] ); 
            end
            ...
            if vel == 0
                dop_tmp(:,1,1) = ones(sample_len,1,1);
            else
        ...
        function [r_coeff, c_ang, res_ang] = FD_channel(obj, sample_len, i_vel)
            ...
            for i = 1:obj.n_path
                ...
                for j = 1:obj.n_ray(i)
                    ...
                    sub_tmp = sub_tmp * obj.pas(ang, j); % apply power term
