...
class GeneratorRev(tf.keras.Model):
    ...
    def call(self, x):
        ...
        for i, decoder_layer in enumerate(self.decoder_layers):
            ...
            ## Concat : Concatenate with the axis of channel
            x = tf.concat([x, encoder_xs[i]], axis=-1)  # skip connect
            # print(x.shape)
...
class GeneratorRev2(tf.keras.Model):
    def __init__(self,
                 n_path=3):

        super(GeneratorRev2, self).__init__()
        self.n_path = n_path * 2
        ...
        p_layer_1 = DecoderLayer(filters=2, kernel_size=4, strides_s=2, apply_dropout=False, add=True)
        # (None, 128, 32, 2)
        p_layer_2 = DecoderLayer(filters=2, kernel_size=4, strides_s=(2, 1), apply_dropout=False, add=True)
        # (None, 256, 32, 2)
        # p_layer_3 = EncoderLayer(filters=2, kernel_size=(6, 1), strides_s=(4, 1),
        #                          apply_batchnorm=False, add=True)
        ...
        encoder_layer_3 = EncoderLayer(filters=64 * 4, kernel_size=4)
        # (None, 8, 4, 256)
        encoder_layer_3n = EncoderLayer(filters=64 * 8, kernel_size=4, strides_s=1)
        # (None, 8, 4, 256)
        encoder_layer_4 = EncoderLayer(filters=64 * 8, kernel_size=4)
        # (None, 4, 2, 512)
        ...
        decoder_layer_1 = DecoderLayer(filters=64 * 8, kernel_size=4, apply_dropout=True)
        # (None, 8, 4, 256)
        decoder_layer_1n = DecoderLayer(filters=64 * 4, kernel_size=4, apply_dropout=True, strides_s=1)
        # (None, 8, 4, 256)
        decoder_layer_2 = DecoderLayer(filters=64 * 4, kernel_size=4, apply_dropout=True)
        # (None, 16, 8, 128)
        decoder_layer_2n = DecoderLayer(filters=64 * 2, kernel_size=4, apply_dropout=True, strides_s=1)
        # (None, 16, 8, 128)
        decoder_layer_3 = DecoderLayer(filters=64 * 2, kernel_size=4, apply_dropout=True)
        # (None, 32, 16, 128)
        decoder_layer_3n = DecoderLayer(filters=64 * 1, kernel_size=4, apply_dropout=True, strides_s=1)
        # (None, 32, 16, 64)
    ...
    def call(self, x):
        ...
        encoder_xs = encoder_xs[:-1][::-1]  # reverse
        # encoder_xs = encoder_xs[::-1]  # reverse
        ## [ENC5, ENC4, ENC3, ENC2]
        # assert (len(encoder_xs) == 4)
